...
function Inventory() {
  const { token } = useAuth();
  const loadCardsByCustomIds = useAction(api.cardStorage.loadCardsByCustomIds);
  const releaseCards = useAction(api.cardStorage.releaseCards);
  const [cardsByCustomId, setCardsByCustomId] = useState<Record<string, any>>({});
  const loadedCustomIdsRef = useRef<Set<string>>(new Set());

  const inventoryQuery = usePaginatedQuery(
    api.userCards.getInventoryPage,
    token ? { token } : "skip",
    { initialNumItems: 50 }
  );

  const inventoryItems = inventoryQuery.results;
  ...
  useEffect(() => {
    ...
    const idsToLoad = inventoryItems
      .map((item) => item.customId)
      .filter((customId) => !loadedCustomIdsRef.current.has(customId));
    ...
    void loadCardsByCustomIds({ customIds: idsToLoad })
      .then((loadedCards) => {
        if (cancelled) {
          if (loadedCards.length > 0) {
            void releaseCards({
              customIds: loadedCards.map((card: any) => card.customId),
            });
          }
          return;
        }

        loadedCards.forEach((card: any) => {
          loadedCustomIdsRef.current.add(card.customId);
        });

        setCardsByCustomId((current) => {
          const next = { ...current };
          loadedCards.forEach((card: any) => {
            next[card.customId] = card;
          });
          return next;
        });
      })
  ...
  }, [inventoryItems, loadCardsByCustomIds, releaseCards, token]);

  useEffect(() => {
    return () => {
      const customIds = Array.from(loadedCustomIdsRef.current);
      if (customIds.length > 0) {
        void releaseCards({ customIds });
      }
      loadedCustomIdsRef.current.clear();
    };
  }, [releaseCards]);
  ...
  const inventoryCards = useMemo(
    () =>
      inventoryItems
        .map((item) => cardsByCustomId[item.customId])
        .filter((card): card is NonNullable<typeof card> => Boolean(card)),
    [cardsByCustomId, inventoryItems]
  );
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">Cards are hydrated from KV in batches of 50.</p>
            </div>
          ...
          {inventoryQuery.status === "LoadingFirstPage" && inventoryCards.length === 0 && (
            <div className="flex justify-center items-center h-64">
              <Loader2 className="h-12 w-12 animate-spin text-red-500" />
            </div>
          )}

          {inventoryQuery.status !== "LoadingFirstPage" && inventoryItems.length === 0 && (
          ...
          {inventoryCards.length > 0 && (
            <>
              ...
              <div className="mt-6 flex justify-center">
                {inventoryQuery.status === "CanLoadMore" && (
                  <Button onClick={() => inventoryQuery.loadMore(50)}>
                    Load 50 More
                  </Button>
                )}
                {inventoryQuery.status === "LoadingMore" && (
                  <Button disabled>
